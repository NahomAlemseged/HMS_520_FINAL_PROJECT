...
# LOADING DATA
############################################################################
df <- read.csv('C:/Users/nahomw/Desktop/assignments/First Quarter/data_wrangling/project_U5M/HMS_520_FINAL_PROJECT/data/logreg_data.csv')
df_lab <- read.csv('C:/Users/nahomw/Desktop/assignments/First Quarter/data_wrangling/project_U5M/HMS_520_FINAL_PROJECT/data/Prep_data.csv')

##
########################################################################
# PART 1: DESCRIPTIVE ANALYSIS AND ASSOCIATION TEST
############################################################################
#label for the variables
###########################################################
df_lab$region <- as.factor(df_lab$region)
df_lab$type_res <- as.factor(df_lab$type_res)
df_lab$bir_int <- as.factor(df_lab$bir_int)
df_lab$fac_del <- as.factor(df_lab$fac_del)
df_lab$outcome <- as.factor(df_lab$outcome)
##############################################################
label(df_lab$outcome)<- "Outcome"
label(df_lab$region)<- "Region"
label(df_lab$type_res)  <- "Residence"
label(df_lab$bir_int)  <- "Birth interval"
label(df_lab$fac_del)  <- "Place of delivery"
label(df_lab$size_child)  <- "size of child"

##########################################################
df_lab$outcome <-factor(df_lab$outcome, 
               levels=c("no","yes"),
               labels=c("Dead","Alive"))
########################################################
...
table1(~ region + type_res + bir_int + fac_del + size_child| outcome, data=df_lab,
...
df_lab$region <- relevel(df_lab$region, ref = "addis adaba")
logistic_model <- glm(outcome ~ type_res+ region + bir_int + fac_del, family = binomial(), df_lab)
